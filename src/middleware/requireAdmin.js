...
export function requireAdmin(req, res, next) {
  if (!config.adminToken) {
    ...
    if (config.isProduction) {
      return res
        ...
        .json({
          error: "unauthorized",
          message: "ניהול לא זמין - לא הוגדר ADMIN_TOKEN",
        });
    }
    if (!warned) {
      console.warn(
        "⚠  ADMIN_TOKEN לא הוגדר - נתיבי הניהול פתוחים ללא הזדהות (פיתוח בלבד).",
      );
      warned = true;
    }
    return next();
  ...
  return res
    .status(401)
    .json({ error: "unauthorized", message: "נדרשת הזדהות מנהל" });
}
